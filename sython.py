...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك المطور سمير ابشر  @DEV_SAMIR')
...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@FTTUTY"))
    except BaseException:
        pass
...
async def _(event):
      await event.reply("""السـورس يعمـل |سيمو
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
...
async def _(event):
      await event.reply("""سمير : @DEV_SAMIR"""
)
